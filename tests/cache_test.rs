...
fn bincode_u32_first_field_is_4_le_bytes() {
    let c = CacheContainer { version: 0xDEADBEEFu32, data: AppData::default() };
...
fn round_trip_valid_version() {
    let original = CacheContainer { version: CACHE_SCHEMA_VERSION, data: AppData::default() };
...
fn mismatched_version_detected_via_prefix() {
    let old = CacheContainer { version: 1, data: AppData::default() };
    let bytes = bincode::serialize(&old).unwrap();
    let version_prefix = u32::from_le_bytes(bytes[..4].try_into().unwrap());
    assert_eq!(version_prefix, 1, "prefix-read must recover written version");
    assert_ne!(version_prefix, CACHE_SCHEMA_VERSION, "old version must differ from current");
}
...
fn corrupted_bytes_fail_deserialize() {
    ...
    assert!(result.is_err(), "random bytes must not deserialize as valid CacheContainer");
}
