...
pub fn ui(f: &mut Frame, app: &mut App) {
    ...
    match app.screen {
        ...
        Screen::CatList => {
            ...
            .highlight_style(Style::default().bg(Color::Rgb(40, 0, 40)).fg(Color::Rgb(255, 0, 255)).add_modifier(Modifier::BOLD));
        ...
        Screen::ChanList => {
            ...
            let items: Vec<ListItem> = if app.filtered.is_empty() {
                ...
                .map(|&idx| {
                    ...
                    if let Some(p) = get_current_epg(ch, &app.data, now) {
                        ...
                        let pct = if total > 0 { (elapsed as f64 / total as f64).clamp(0.0, 1.0) } else { 0.0 };
            ...
            let list = List::new(items)
                ...
                .highlight_style(Style::default().bg(Color::Rgb(0, 40, 40)).fg(Color::Cyan).add_modifier(Modifier::BOLD));
        ...
        Screen::RadioList => {
            ...
            let category = if app.data.radio_groups.is_empty() { "All" } else { &app.data.radio_groups[cat_idx] };
            
            let filtered_radio: Vec<_> = app.data.radio.iter().filter(|r| {
                category == "All" || r.genres.iter().any(|g| g.to_uppercase() == category.to_uppercase())
            }).collect();
            ...
            let items: Vec<ListItem> = filtered_radio
                ...
                .map(|r| {
                    let mut spans = vec![
                        Span::styled(format!("[{}] ", &r.provider[..1]), Style::default().fg(if r.provider == "Record" { Color::Rgb(255, 0, 255) } else { Color::Yellow })),
                        Span::styled(format!("{} ", r.title), Style::default().fg(Color::White).bold()),
                    ];
            ...
            let list = List::new(items)
                ...
                .highlight_style(Style::default().bg(Color::Rgb(20, 20, 40)).fg(Color::White).add_modifier(Modifier::BOLD));
            f.render_stateful_widget(list, area, &mut app.r_state);
        }
        Screen::LocalList => {
            let items: Vec<ListItem> = app.local_files.iter().map(|p| ListItem::new(format!("📄 {}", p.file_name().unwrap_or_default().to_string_lossy()))).collect();
            let list = List::new(items).block(Block::default().title(" Local Playlists ")).highlight_style(Style::default().bg(Color::Rgb(20, 60, 20)));
            f.render_stateful_widget(list, area, &mut app.l_state);
        }
        Screen::Detail => {
            if app.filtered.is_empty() { f.render_widget(Paragraph::new("No Data"), area); return; }
            ...
            if let Some(id) = find_epg_id(ch, &app.data) {
                if let Some(progs) = app.data.epg.get(&id) {
                    ...
                    for (i, p) in progs.iter().enumerate() {
                        ...
                        let (icon, style) = if p.start > now { ("📅", Style::default().fg(Color::DarkGray)) } else if p.stop < now { ("⏪", Style::default().fg(Color::Green)) } else { ("🔴", Style::default().fg(Color::Magenta).bold()) };
                        items.push(ListItem::new(Line::from(vec![Span::styled(format!("{} {} ", icon, time_str), style), Span::styled(&p.title, Style::default().fg(Color::White))])));
                        
                        if i == sel_idx {
                            selected_title = p.title.clone();
                            selected_desc = if p.desc.is_empty() { "No description available.".to_string() } else { p.desc.clone() };
                        }
                    }
                }
            }
            if !found_epg { items.push(ListItem::new("No EPG Data")); }

            let width = area.width.saturating_sub(2).max(1) as usize;
            let desc_lines: u16 = selected_desc.lines()
                .map(|l| (l.chars().count() as f64 / width as f64).ceil() as u16)
                .sum::<u16>()
                .max(1);
            let height = (desc_lines + 2).min(area.height / 2).max(3);

            let chunks = Layout::default().constraints([Constraint::Length(3), Constraint::Min(0), Constraint::Length(height)]).split(area);
            let header_spans = vec![Span::styled(format!("📺 {}", ch.name), Style::default().fg(Color::Cyan).bold())];
            f.render_widget(Paragraph::new(Line::from(header_spans)), chunks[0]);

            let list = List::new(items).block(Block::default().title(" Schedule ").borders(Borders::ALL)).highlight_style(Style::default().bg(Color::Rgb(40, 40, 60)));
            f.render_stateful_widget(list, chunks[1], &mut app.d_state);
            
            let desc_block = Block::default().title(format!(" Info: {} ", selected_title)).borders(Borders::ALL).border_style(Style::default().fg(Color::Yellow));
            f.render_widget(Paragraph::new(selected_desc).block(desc_block).wrap(Wrap { trim: true }), chunks[2]);
        }
        Screen::Settings => {
            let items = [format!("Playlist: {}", app.config.playlist_url), format!("EPG: {}", app.config.epg_url), "Save & Exit".into()];
            let list = List::new(items.iter().map(|i| ListItem::new(i.as_str())).collect::<Vec<_>>()).highlight_style(Style::default().bg(Color::Yellow).fg(Color::Black));
